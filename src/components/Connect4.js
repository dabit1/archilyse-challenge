...
const Connect4 = React.forwardRef(({ onGameFinish, numRows, numCols }, ref) => {
  const [game, setGame] = React.useState(createGame(numRows, numCols));
  const [gameFinished, setGameFinished] = React.useState(false);
  const [board, setBoard] = React.useState(game.board);
  const [currentPlayer, setCurrentPlayer] = React.useState(game.currentPlayer);
  ...
  const handleBoardAddPiece = React.useCallback(
    ...
    [game, gameFinished, onGameFinish]
  );
